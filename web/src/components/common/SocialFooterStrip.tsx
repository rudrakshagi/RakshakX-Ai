...
export const SocialFooterStrip: React.FC<{ className?: string }> = ({ className = '' }) => {
  const socialLinks = [
    ...
    {
      name: 'Instagram',
      url: 'https://www.instagram.com/rudraksh_agi/',
      icon: (
        <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
    ...
    {
      name: 'YouTube',
      url: 'https://youtube.com/@rudraksh_agi',
      icon: (
        <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
    ...
    {
      name: 'GitHub',
      url: 'https://github.com/rudrakshagi',
      icon: (
        <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
      ...
      bg: 'hover:bg-slate-900 hover:text-white dark:hover:bg-white dark:hover:text-black',
    },
    {
      name: 'LinkedIn',
      url: 'https://www.linkedin.com/in/rudraksh_agi/',
      icon: (
        <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
      ...
      <div className="flex items-center gap-2.5 sm:gap-3">
        {socialLinks.map((s) => (
          <a
            ...
            title={s.name}
            aria-label={s.name}
            className={`w-9 h-9 rounded-full bg-white dark:bg-[#121B2D] border border-slate-200 dark:border-slate-700 shadow-xs flex items-center justify-center text-slate-600 dark:text-slate-300 transition-all duration-200 hover:scale-110 active:scale-95 ${s.bg}`}
          >
            {s.icon}
          </a>
        ))}
